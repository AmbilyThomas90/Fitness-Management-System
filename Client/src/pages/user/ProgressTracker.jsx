...
const ProgressTracker = () => {
  ...
  // Initialize as true so the spinner shows immediately 
    ...
    const fetchGoals = async () => {
      try {
        ...
        if (fetchedGoals.length > 0) {
          setSelectedGoal(fetchedGoals[0]._id);
        } else {
          // If no goals exist, stop loading so empty message can show
  ...
  // Fetch history when goal changes
  useEffect(() => {
    if (selectedGoal) {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-8 p-6 bg-white shadow-lg rounded-2xl">
      ...
      <form
        ...
        className="space-y-5 mb-10 bg-gray-50 p-5 rounded-xl shadow-inner"
      >
        {/* Select Goal */}
        <div>
      ...
      <div className="history">
        <h3 className="font-bold border-b pb-2 mb-5 text-gray-800">
          Recent History
        </h3>
        ...
        {historyLoading ? (
          ...
          <div className="space-y-4">
            {history.map((log) => (
              <div
                key={log._id}
                className="flex flex-col sm:flex-row sm:justify-between p-4 border-l-4 border-green-500 bg-white shadow-sm rounded-lg"
              >
